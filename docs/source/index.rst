...
Sticker can be downloaded from Forgebox_. Once downloaded and unpacked, create a mapping to the sticker directory called '/sticker' (not required if you unpacked sticker to the webroot).
...
You may wish to conditionally include a file for IE 7 and below, or specify that a stylesheet is only for *print*. To do so, you can make use of the :code:`setIE()` and :code:`setMedia()` methods:
